...
-- Third: create assignee id association table

create table eager_assignee_id_lookup
as 
select distinct firm, lookup_firm, id from 
		(select ea.firm, ea.lookup_firm, ac.id
		from eager_assignee ea 
		inner join assignee_clnd ac on ea.lookup_firm = ac.organization_clnd -- returns 4,051
		) as temp1;

select * from eager_assignee_id_lookup;
select count(*) from eager_assignee;
select * from eager_assignee where lookup_firm not in 
(select lookup_firm from eager_assignee_id_lookup);
select count(distinct lookup_firm) from eager_assignee_id_lookup;
...
select count(p.id), eail.lookup_firm -- or number?
from patent p, patent_assignee pa, eager_assignee_id_lookup eail
where eail.id = pa.assignee_id
and pa.patent_id = p.id
group by eail.lookup_firm;

-- count number of patents (just those falling in the three industries), group by organization 
select count(epa.id), eail.final_lookup_firm_clnd -- or number?
from eager_patent_all epa, eager_assignee_id_lookup eail, patent_assignee pa
where epa.id = pa.patent_id
and pa.assignee_id = eail.id
group by eail.final_lookup_firm_clnd;
